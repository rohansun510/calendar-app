...
export default function SettingsDialog({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center">
      ...
      <div className="relative w-[420px] glass-window rounded-xl animate-scale-in overflow-hidden" style={{ maxHeight: '85vh', display: 'flex', flexDirection: 'column' }}>
        ...
        <div className="p-5 space-y-6" style={{ overflowY: 'auto', flex: 1, minHeight: 0 }}>
          {/* 主题色 */}
          <section>
          ...
          <section>
            ...
            <div className="flex gap-2">
              {([{ value: 'left' as const, label: '左侧' }, { value: 'right' as const, label: '右侧' }]).map(({ value, label }) => (
                <button key={value} onClick={() => update({ sidebarPosition: value })}
                  className={`flex-1 py-2.5 text-sm rounded-lg border transition-all ${
                    settings.sidebarPosition === value ? 'border-[rgb(var(--theme-color))] bg-[rgb(var(--theme-color))/0.1]' : 'border-[rgba(var(--border-color)/0.25)] text-[rgb(var(--text-secondary))]'}`}>
                  {label}
                </button>
              ))}
            </div>
          ...
          <section className="text-center text-xs text-[rgb(var(--text-tertiary))]">日历清单 v1.1.0 · 纯本地存储，数据安全</section>
